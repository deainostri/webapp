...
import NewPresalePageLoggedIn from "./NewPresalePageLoggedIn";
...
const NewPresalePage = observer((props: any) => {
  //
  ...
  const renderContent = () => {
    if (isLoggedIn) {
      return <NewPresalePageLoggedIn skipTimeLeft={skipTimeLeft} />;
    }

    return <PresalePageLoggedOut />;
  };
...
const PresalePageLoggedOut = observer(() => {
  //
  const store = useRootStore();
